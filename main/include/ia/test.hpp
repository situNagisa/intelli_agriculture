...
{
	auto keyboard = ia::detail::factory::create_keyboard();
	auto temperature = ia::detail::factory::create_temperature();

	ia::api::hw::sensors::temperature_t prevent{};

	while (true)
	{
		keyboard->update();
		if (!keyboard->is_raised(ia::api::hw::keyboard_code::up))
			continue;

		//temperature->_reset();
		//NGS_LOGL(info, "check ", temperature->_check());
		auto&& [h, t] = temperature->_read_data();
		NGS_LOGL(info, "h ", h, ", t ", t);
	}
}
